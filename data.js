...
// The data has been synthetically expanded to simulate a large catalog (~10000 planets)
...
for (let i = 0; i < 5; i++) {
    basePlanetData.forEach(planet => {
        ...
        newPlanet.pl_rade *= (1 + (Math.random() - 0.5) * 0.2);
        newPlanet.pl_insol *= (1 + (Math.random() - 0.5) * 0.2);
        newPlanet.pl_orbper *= (1 + (Math.random() - 0.5) * 0.2);
        newPlanet.pl_eqt = newPlanet.pl_eqt ? newPlanet.pl_eqt + Math.round((Math.random() - 0.5) * 50) : null;
...
for (let i = 0; i < 50; i++) {
    ...
    expandedData.push({
        ...
        pl_name: `K-RAND-${Math.floor(Math.random() * 9000) + 1000}.${String(i+1).padStart(2,'0')}`,
        pl_rade: Math.random() * 3 + 0.5,
        pl_insol: Math.random() * 2 + 0.1,
        pl_orbper: Math.random() * 400 + 10,
        pl_eqt: Math.floor(Math.random() * 600) + 150,
    });
}

// Now generate a large number of additional candidates
const NUM_NEW_PLANETS = 14900;
...
function getRandomValue(prop) {
    ...
    // for planet sizes, star sizes, etc.
    const skew = (prop === 'pl_rade' || prop === 'st_rad' || prop === 'pl_masse' || prop === 'st_mass') ? 2.5 : 2;
    return range.min + (Math.pow(Math.random(), skew)) * (range.max - range.min);
}

for (let i = 0; i < NUM_NEW_PLANETS; i++) {
    expandedData.push({
        pl_name: `KIC-${Math.floor(Math.random() * 9000000) + 1000000}.01`,
        ...
        disc_year: Math.floor(Math.random() * (2018 - 2009 + 1)) + 2009,
